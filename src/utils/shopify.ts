...
export default async function getHeroBanner() {
  const data = await makeShopifyRequest(HeroBannerQuery);
  const { metaobjects } = data;
  const {edges} = metaobjects;
  const bannerObjects = edges[0].node.fields[0].references.nodes;
